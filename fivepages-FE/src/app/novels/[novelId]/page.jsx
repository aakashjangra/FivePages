...
export default function NovelPage() {
  ...
  useEffect(() => {
    const fetchNovel = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5000/api/v1/novels/${novelId}`);
        const data = await response.json();
        console.log("API Response:", data); // Debug log
  
        if (!response.ok) throw new Error(data.message || "Failed to fetch novel data.");
  ...
  const readNow = () => {
    router.push(`/chapter/${novel.chapters[0]?.id || 1}`);
  };
      ...
      <div className="w-full max-w-3xl p-10 bg-white shadow-md border border-gray-300 rounded-xl space-y-10">
        ...
        <div className="flex gap-8">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-gray-600 mt-3"><span className="font-semibold">PublishedYear:</span> {novel.publishedYear}</p>
            ...
            <p className="text-gray-600 mt-3"><span className="font-semibold">Total Chapters:</span> {novel.totalChapters}</p>
          </div>
        </div>
          ...
          <ul className="space-y-4">
            {novel.chapters.map((chapter, index) => (
              <li key={chapter.id} className="flex items-center">
                ...
                <Link href={`/chapter/${chapter.id}`} className="text-blue-600 hover:underline">
                  {chapter.title}
                </Link>
              </li>
            ))}
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-gray-300 space-y-5">
          ...
          <textarea className="w-full p-5 border border-gray-300 rounded-lg" placeholder="Write your comment..." value={newComment} onChange={(e) => setNewComment(e.target.value)}></textarea>
          <button className="px-6 py-4 bg-blue-500 hover:bg-blue-700 text-white rounded-lg" onClick={toggleLike}>
            Post Comment
          </button>
        </div>
      </div>
